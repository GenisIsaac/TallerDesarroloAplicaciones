...
function selectRubric(type) {
    ...
    showNotification(rubrics[type] || 'Rúbrica seleccionada', 'info');
}
...
function selectAnnotationTool(tool) {
    ...
    showNotification(tools[tool] || 'Herramienta seleccionada', 'info');
}
...
function openThesisViewer(thesisId, asignacionId) {
    ...
    // Mostrar carga
    const loadingDocument = document.getElementById('loadingDocument');
    if (loadingDocument) loadingDocument.style.display = 'block';
    
    // Actualizar título del modal
    const modalTitle = document.querySelector('#thesisViewerModal .modal-title');
    if (modalTitle) modalTitle.textContent = `Revisando Tesis #TH-${thesisId}`;
    
    // Simular carga
    setTimeout(() => {
        if (loadingDocument) loadingDocument.style.display = 'none';
        if (textViewer) {
            textViewer.innerHTML = `
                <div class="p-6">
                    <h4 class="text-lg font-semibold text-gray-900 mb-4">Documento de Tesis</h4>
                    <p class="text-gray-700 mb-4">Contenido del documento de tesis #TH-${thesisId}</p>
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <p class="text-sm text-gray-600">Este es un simulador del visor de documentos.</p>
                    </div>
                </div>
            `;
            textViewer.style.display = 'block';
...
function openChatWithStudent(studentName, asignacionId, estudianteId, docenteId, thesisTitle) {
    // Guardar datos actuales
    currentChatData = {
        asignacionId: asignacionId,
        estudianteId: estudianteId,
        docenteId: docenteId,
        estudianteName: studentName,
    ...
    const chatThesisTitle = document.getElementById('chatThesisTitle');
    const chatStudentAvatar = document.getElementById('chatStudentAvatar');
    
    if (chatStudentName) chatStudentName.textContent = studentName;
    if (chatThesisTitle) chatThesisTitle.textContent = thesisTitle || 'Sin título';
    if (chatStudentAvatar) chatStudentAvatar.src = 'https://images.unsplash.com/photo-1584824486509-112e4181ff6b?q=80&w=2940&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D';
...
function loadChatMessages() {
    if (!currentChatData.asignacionId) return;
    
    fetch(`MensajeController?action=obtener&idAsignacion=${currentChatData.asignacionId}&lastId=${currentChatData.lastMessageId}`)
        .then(response => {
        ...
        .then(messages => {
            ...
            if (messages && messages.length > 0) {
                ...
                messages.forEach(message => {
                    ...
                    if (!document.querySelector(`[data-message-id="${message.id}"]`)) {
                        ...
                        // Si es el primer mensaje, limpiar el contenedor
                        ...
                        if (message.id > currentChatData.lastMessageId) {
                            currentChatData.lastMessageId = message.id;
                        }
                    }
                });
            ...
            } else if (messages && messages.length === 0 && currentChatData.lastMessageId === 0) {
                // Si no hay mensajes y es la primera carga
                const chatContainer = document.getElementById('chatMessages');
                if (chatContainer && chatContainer.children.length === 1 && 
                    chatContainer.children[0].classList.contains('text-center')) {
                    chatContainer.innerHTML = `
        ...
        .catch(error => {
            ...
            // Mostrar estado de error solo si no es la primera carga
            if (currentChatData.lastMessageId > 0) {
                showNotification('Error al cargar nuevos mensajes', 'error');
            }
        });
}

function createMessageElement(message) {
    const isDocente = message.idDocente === currentChatData.docenteId;
    const fecha = new Date(message.fechaEnvio || new Date());
    const timeString = fecha.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    const dateString = fecha.toLocaleDateString();
    
    const div = document.createElement('div');
    div.className = `mb-4 ${isDocente ? 'text-right' : 'text-left'} new-message`;
    div.dataset.messageId = message.id;
    
    // Determinar color según tipo de mensaje
    let bgColor = isDocente ? 'bg-blue-100' : 'bg-gray-100';
    let textColor = isDocente ? 'text-blue-900' : 'text-gray-900';
    let timeColor = isDocente ? 'text-blue-600' : 'text-gray-600';
    
    if (message.tipoMensaje === 'urgente') {
        bgColor = 'bg-red-100';
        textColor = 'text-red-900';
        timeColor = 'text-red-600';
    } else if (message.tipoMensaje === 'sistema') {
    ...
    div.innerHTML = `
        <div class="inline-block max-w-xs lg:max-w-md ${bgColor} ${textColor} rounded-lg p-3 shadow-sm">
            $/{!isDocente ? `<div class="text-xs font-medium mb-1">${currentChatData.estudianteName}</div>` : ''}
            <p class="text-sm break-words">${escapeHtml(message.contenido)}</p>
            <div class="flex justify-between items-center mt-2">
                <span class="text-xs ${timeColor}">${timeString}</span>
                $/{message.estado === 'leido' ? 
                    '<span class="text-xs text-green-600 ml-2" title="Leído">✓✓</span>' : 
                    '<span class="text-xs text-gray-400 ml-2" title="Enviado">✓</span>'}
            </div>
        </div>
...
function sendChatMessage() {
    ...
    if (!message || !currentChatData.asignacionId) {
        return;
    }
    ...
    formData.append('idAsignacion', currentChatData.asignacionId);
    formData.append('idEstudiante', currentChatData.estudianteId);
    formData.append('asunto', 'Mensaje de chat');
    formData.append('contenido', message);
    formData.append('tipoMensaje', 'chat');
    
    // Enviar mensaje
    fetch('MensajeController', {
        method: 'POST',
        body: formData
    })
    ...
    .then(result => {
        if (result.success) {
            ...
            showNotification('Error al enviar mensaje: ' + (result.error || 'Error desconocido'), 'error');
        }
    })
    .catch(error => {
        console.error('Error:', error);
...
function markMessagesAsRead() {
    ...
    fetch('MensajeController', {
        ...
        body: `action=marcarLeidoPorAsignacion&idAsignacion=${currentChatData.asignacionId}&idEstudiante=${currentChatData.estudianteId}`
    })
    .then(response => response.json())
    .then(result => {
        if (!result.success) {
...
function startChatPolling() {
    ...
    currentChatData.pollInterval = setInterval(() => {
        loadChatMessages();
        checkTypingStatus();
    }, 3000);
    ...
    if (chatStatus && connectionStatus) {
        chatStatus.classList.remove('hidden');
        connectionStatus.textContent = 'Conectado';
        chatStatus.className = 'text-xs text-center p-2 bg-green-100 text-green-800 border-b';
    }
}

function stopChatPolling() {
    if (currentChatData.pollInterval) {
        clearInterval(currentChatData.pollInterval);
        currentChatData.pollInterval = null;
    }
    
    // Ocultar estado de conexión
    const chatStatus = document.getElementById('chatStatus');
...
function checkTypingStatus() {
    const now = Date.now();
    // Si han pasado más de 5 segundos desde el último typing, ocultar indicador
    if (now - currentChatData.lastTypingCheck > 5000) {
        hideTypingIndicator();
    }
}
...
function handleChatKeyDown(event) {
    // Si es Enter sin Shift, enviar mensaje
    ...
    // Notificar typing (cada 2 segundos como máximo)
    const now = Date.now();
    if (now - currentChatData.lastTypingCheck > 2000) {
        notifyTyping();
        currentChatData.lastTypingCheck = now;
    }
}

function handleChatFocus() {
    // Mostrar que el docente está activo en el chat
    if (currentChatData.asignacionId) {
...
function notifyTyping() {
    // Enviar notificación de typing al servidor
    if (currentChatData.asignacionId && currentChatData.isTyping === false) {
        currentChatData.isTyping = true;
        ...
        fetch('MensajeController', {
            ...
            body: `action=typing&idAsignacion=${currentChatData.asignacionId}&idDocente=${currentChatData.docenteId}&typing=true`
        });
        
        // Resetear después de 3 segundos
        currentChatData.typingTimeout = setTimeout(() => {
            currentChatData.isTyping = false;
        }, 3000);
    }
}
...
function sendTypingNotification(isTyping) {
    ...
    fetch('MensajeController', {
        ...
        body: `action=typing&idAsignacion=${currentChatData.asignacionId}&idDocente=${currentChatData.docenteId}&typing=${isTyping}`
    });
}

function showTypingIndicator(studentName) {
    const typingStudentName = document.getElementById('typingStudentName');
    const typingIndicator = document.getElementById('typingIndicator');
    
    if (typingStudentName) typingStudentName.textContent = studentName;
...
function updateLastUpdateTime() {
    ...
    if (lastUpdate) lastUpdate.textContent = `Actualizado: ${timeString}`;
}
...
// ========== FUNCIONES DE SESIÓN ==========
function openNewSessionFromChat() {
    closeChatModal();
    openNewSessionModal(currentChatData.estudianteName, null, currentChatData.asignacionId);
}

// ========== FUNCIONES ESPECÍFICAS PARA ASESORÍA ==========
function openAsesorViewer(thesisId, asignacionId, studentName) {
    const asesorThesisId = document.getElementById('asesorThesisId');
    if (asesorThesisId) asesorThesisId.textContent = thesisId;
    openModal('asesorViewerModal');
    
    // Mostrar información del estudiante
    showNotification(`Viendo tesis como asesor del estudiante: ${studentName}`, 'info');
    
    // Simular carga de documento
    setTimeout(() => {
        const asesorLoadingDocument = document.getElementById('asesorLoadingDocument');
        const asesorTextViewer = document.getElementById('asesorTextViewer');
        
        if (asesorLoadingDocument) asesorLoadingDocument.style.display = 'none';
        if (asesorTextViewer) {
            asesorTextViewer.innerHTML = `
                <div class="p-6">
                    <h4 class="text-lg font-semibold text-primary mb-4">Documento de Tesis - Vista Asesor</h4>
                    <div class="bg-gray-50 p-4 rounded-lg mb-4">
                        <p class="text-sm text-gray-700"><strong>Estudiante:</strong> ${studentName}</p>
                        <p class="text-sm text-gray-700"><strong>Tesis ID:</strong> #TH-${thesisId}</p>
                        <p class="text-sm text-gray-700"><strong>Rol:</strong> Asesor</p>
                    </div>
                    <p class="text-gray-700 mb-4">Como asesor, tienes acceso a:</p>
                    <ul class="list-disc pl-5 text-gray-700 mb-4">
                        <li>Revisión completa del documento</li>
                        <li>Sugerencias de mejora</li>
                        <li>Comentarios constructivos</li>
                        <li>Guía metodológica</li>
                        <li>Evaluación de avances</li>
                    </ul>
                    <button onclick="simularCargaDocumentoAsesor()" class="btn-primary">
                        Cargar Documento Completo
                    </button>
                </div>
            `;
            asesorTextViewer.style.display = 'block';
        }
    }, 1000);
}

function simularCargaDocumentoAsesor() {
    const asesorTextViewer = document.getElementById('asesorTextViewer');
    if (asesorTextViewer) {
        asesorTextViewer.innerHTML = `
            <div class="p-6">
                <h4 class="text-lg font-semibold text-primary mb-4">Documento de Tesis Completo</h4>
                <div class="space-y-4">
                    <div class="border border-gray-200 rounded p-4">
                        <h5 class="font-medium mb-2">Capítulo 1: Introducción</h5>
                        <p class="text-sm text-gray-600">Texto completo del capítulo 1...</p>
                    </div>
                    <div class="border border-gray-200 rounded p-4">
                        <h5 class="font-medium mb-2">Capítulo 2: Marco Teórico</h5>
                        <p class="text-sm text-gray-600">Texto completo del capítulo 2...</p>
                    </div>
                    <div class="border border-gray-200 rounded p-4">
                        <h5 class="font-medium mb-2">Capítulo 3: Metodología</h5>
                        <p class="text-sm text-gray-600">Texto completo del capítulo 3...</p>
                    </div>
                </div>
            </div>
        `;
...
function sugerirCorrecciones() {
    const sugerencias = [
        "Revisar la estructura del capítulo 2",
        "Incluir más referencias actualizadas",
        "Mejorar la redacción de la metodología",
        "Ampliar el análisis de resultados"
    ];
    
    showNotification(`Sugerencias de corrección disponibles: ${sugerencias.length}`, 'info');
    ...
    if (asesorTextViewer) {
        asesorTextViewer.innerHTML += `
            <div class="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <h5 class="font-medium text-yellow-800 mb-2">Sugerencias de Corrección:</h5>
...
function agregarComentarioAsesor() {
    const comentario = prompt("Ingresa tu comentario constructivo para el estudiante:");
    if (comentario) {
        showNotification("Comentario guardado para el estudiante", 'success');
    }
}

function verAvances() {
    showNotification("Cargando historial de avances...", 'info');
}

function descargarParaAsesor() {
    showNotification("Descargando documento con anotaciones de asesor", 'info');
    setTimeout(() => {
        showNotification("Documento descargado exitosamente", 'success');
    }, 1500);
}

function guardarRevisionAsesor() {
    if (confirm("¿Deseas guardar tu revisión como asesor?")) {
        showNotification("Revisión guardada exitosamente", 'success');
        closeModal('asesorViewerModal');
    }
}
...
function openTemplate(templateType) {
    const templates = {
        'metodologia': {
            title: 'Plantillas de Metodología',
            desc: 'Formatos para estructura metodológica'
        },
        'cronograma': {
            title: 'Cronogramas de Investigación',
            desc: 'Plantillas para planificación temporal'
        },
        'evaluacion': {
            title: 'Formatos de Evaluación',
            desc: 'Rúbricas y checklists de evaluación'
        }
    };
    
    const template = templates[templateType];
    if (template) {
        showNotification(`Abriendo ${template.title}`, 'info');
        
        setTimeout(() => {
            const decision = confirm(`¿Deseas descargar la ${template.title.toLowerCase()}?`);
            if (decision) {
                showNotification(`${template.title} descargada exitosamente`, 'success');
            }
        }, 500);
    }
}

function verHistorialSesiones(asignacionId) {
    openModal('sesionesModal');
    
    // Simular carga de sesiones
    const sesiones = [
        { fecha: '2024-01-15', duracion: '60 min', tipo: 'Revisión metodología', estado: 'Completada' },
        { fecha: '2024-01-08', duracion: '45 min', tipo: 'Avances capítulo 2', estado: 'Completada' },
        { fecha: '2024-01-02', duracion: '90 min', tipo: 'Planeación general', estado: 'Completada' }
    ];
    
    const sesionesList = document.getElementById('sesionesList');
    if (sesionesList) {
        sesionesList.innerHTML = sesiones.map(sesion => `
            <div class="border border-gray-200 rounded-lg p-4">
                <div class="flex justify-between items-center mb-2">
                    <h4 class="font-medium text-gray-900">${sesion.tipo}</h4>
                    <span class="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full">${sesion.estado}</span>
                </div>
                <div class="flex justify-between text-sm text-gray-600">
                    <span>Fecha: ${sesion.fecha}</span>
                    <span>Duración: ${sesion.duracion}</span>
                </div>
            </div>
        `).join('');
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (searchStudent) {
        searchStudent.addEventListener('input', filtrarEstudiantes);
    }
    
    const filterStatusAdvising = document.getElementById('filterStatusAdvising');
    if (filterStatusAdvising) {
        filterStatusAdvising.addEventListener('change', filtrarEstudiantes);
    }
    ...
    // Configurar eventos de teclado para el chat
