...
public static boolean crearMensaje(Mensaje mensaje) {
    ...
    public List<Mensaje> obtenerMensajesPorEstudiante(int estudianteId) {
    List<Mensaje> mensajes = new ArrayList<>();
    String sql = "SELECT m.*, " +
                 "d.nombre || ' ' || d.apellido as docente_nombre, " +
                 "e.nombre || ' ' || e.apellido as estudiante_nombre, " +
                 "CASE WHEN m.id_docente IS NOT NULL THEN 'DOCENTE' ELSE 'ESTUDIANTE' END as remitente_tipo " +
                 "FROM mensaje m " +
                 "LEFT JOIN usuario d ON m.id_docente = d.id " +
                 "LEFT JOIN usuario e ON m.id_estudiante = e.id " +
                 "WHERE m.id_estudiante = ? OR (m.id_docente IS NOT NULL AND EXISTS " +
                 "  (SELECT 1 FROM asignacion a WHERE a.id = m.id_asignacion AND a.id_estudiante = ?)) " +
    ...
    try (Connection conn = DatabaseConnection.getConnection();
        ...
        while (rs.next()) {
            ...
            Integer idDocente = rs.getObject("id_docente", Integer.class);
            mensaje.setIdDocente(idDocente);
            
            Integer idEstudiante = rs.getObject("id_estudiante", Integer.class);
            ...
            // Nuevos campos
            mensaje.setNombreDocente(rs.getString("docente_nombre"));
            mensaje.setNombreEstudiante(rs.getString("estudiante_nombre"));
    ...
    // Método para obtener todos los mensajes de una asignación (sin lastId)
    public List<Mensaje> obtenerTodosMensajesPorAsignacion(int idAsignacion) {
        List<Mensaje> mensajes = new ArrayList<>();
        String sql = "SELECT * FROM mensajes WHERE id_asignacion = ? ORDER BY fecha_envio ASC";
    ...
    // Método para marcar mensajes como leídos por asignación
    public boolean marcarComoLeidoPorAsignacion(int idAsignacion, int idUsuario) {
        String sql = "UPDATE mensajes SET estado = 'leido' WHERE id_asignacion = ? AND id_estudiante = ? AND estado = 'no_leido'";
